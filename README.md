<img src="https://capsule-render.vercel.app/api?type=slice&color=pink&height=100&section=header&text=Ukki&fontSize=70" />
...
### 로그인
    ...
    * 이메일 인증과 아이디 찾기에서 찾은 아이디를 통해 새로운 비밀번호 변경이 가능하다.

### 식당
    * 식당 전체 조회가 가능하다.
    * 식당에 대한 소개, 사진, 리뷰 등의 상세 조회가 가능하다.
    * 예약 가능 날짜를 확인하고 예약이 가능하다. (Default : Today)
    * 날짜를 선택하고 시간을 선택하는 순서를 가지며 예약을 마친다.
    * 예약을 마치면 안내문구가 나타나고 예약확인을 할 수 있다.
    * 예약 확인을 누르면 사용자의 정보, 예약정보가 예약 DB에 저장된다.
    * 식당에서는 식당 리뷰에 문제가 있는 경우 관리자 측으로 신고버튼을 통해 신고할 수 있다.
    * 식당 상세 조회에서는 예약과 함께 식당에 대한 리뷰를 확인할 수 있다.

### 메인 페이지
    * 검색창 클릭 시 검색 페이지로 이동
    * 제휴된 가게들 대표사진들과 상표명 나열 (배너식 슬라이드 방식)
    * 음식 종류별 카테고리를 제공 (클릭 시 위치기반 가까운 리스트 조회)
    * 자신의 위치를 누르면 가게의 위치가 표시된 지도 조회 가능
    * 사용자의 동의를 얻고(주의 문구) 위치 기반으로 랜덤한 식당을 자동 예약해주는 시스템 - 주의 문구에 취소 불가 및 바로 예약을 알림
...
### 플로팅 버튼
    * 주요 기능
    ...
    2. 스크롤 탑
    3. 문의하기
        -> 문의하기에서 데이터베이스 기반 번호를 고르는 것에 따라 안내문구가 나타나고 자세한 문의 버튼을 넣어 1:1 문의로
            이동 가능 (예약, 환불, 기타 등)
            
### 문의 페이지
    * 문의 전체 확인, 수정, 제거 가능
    * 각 카테고리별 기본 문의 양식은 다르게 나타남

### 마이 페이지
    * 회원 정보 열람 및 수정, 탈퇴 가능
    * 등급은 예약 횟수에 따라 분류되며 등급에 따라 뱃지 부여 및 혜택 제공
    * 자신이 문의 한 내용에 대한 정보 열람 가능

### 가게 페이지 (제휴 맺은 식당 주인 측)
    * 자신이 등록한 가게에 대한 정보 열람 가능
    * 시간별로 예약 가능 인원 조정 가능
    * 가게 예약 현황 확인 가능
    * 자신의 가게 리뷰에 대한 신고 가능(관리자 측에 알림)
    * 관리자에게 문의 가능

### 관리자 페이지
    * 식당에 대한 전체조회, 상세조회, 변경, 제거 가능
    * 리뷰에 대한 전체조회, 상세조회, 변경, 제거 가능
    * 회원에 대한 전체조회, 상세조회, 변경, 제거 가능
    * 가게를 선택해 해당 가게 실적 파일 출력 가능
    * 리뷰 신고에 대한 조회 및 삭제 가능
